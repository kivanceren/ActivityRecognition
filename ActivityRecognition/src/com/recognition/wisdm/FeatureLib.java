...
public class FeatureLib {
	...
	public static double[] generic_bins = {
	 ...
	 * @return
	 */
	public static TupFeat processTup(int count, long[] t, double[] x,
			double[] y, double[] z, TupFeat tup, double[] bins){
		...
		floatFeat[39] = getSDiv(count, x, floatFeat[30]);
		floatFeat[40] = getSDiv(count, y, floatFeat[31]);
		floatFeat[41] = getSDiv(count, z, floatFeat[32]);
	 ...
	 *            the array
	 * @param act
	 *            the activity happening: used to select which bins to use
	 * @param axis
	 *            which axis we're looking at, used to select bins
	 * @return an array of %'s
	 */
	public static double[] fillBins(int count, double[] n, char axis, double[] bins) {
